...
TIENDA_API_URL = os.environ.get(
    "TIENDA_API_URL",
    "https://tienda-api-1079186964678.us-central1.run.app/v1/tienda/search"
)
...
def call_tienda_api(normalized_query: str) -> dict:
    """Proxy hacia la Tienda API (Cloud Run).
    Espera: GET /v1/tienda/search?q=...
    """
    r = requests.get(
        TIENDA_API_URL,
...
BRAND_ONLY_BLOCK_TERMS = set([
    # accesorios comunes
    "watch","smartwatch","reloj","reloj inteligente","band","pulsera",
    "airpods","audifonos","audífonos","earbuds","cargador","cable","funda","case",
    "power bank","powerbank","bateria externa","batería externa","tablet","ipad",
    # hogar / tienda fija
    "router","modem","módem","deco","decodificador","fibra","wifi","internet hogar"
])
...
- "pelicula": intención de entretenimiento en Claro video: título de película O serie, franquicia/saga, personaje/animación("Bob Esponja", "Dragon Ball"), género/categoría ("películas de terror", "películas románticas"), o búsquedas típicas para ver contenido.
  Si la consulta parece de entretenimiento/streaming, responde "pelicula" aunque no sea un título exacto.
- "general": Preguntas de soporte, trámites, preguntas informativas u otros no cubiertos.
...
def query_prod():
    ...
    try:
        ...
        if respuesta is None and category == "tienda_productos":
            # Fuente de verdad: Tienda API (Cloud Run)
            try:
                respuesta = call_tienda_api(normalized_query)
            except Exception as e:
                logger.exception("Error llamando Tienda API", exc_info=e)
                respuesta = {
                    "titulo": f"Ups, no encontramos '{normalized_query}' por ahora",
                    "descripcion": "No pudimos consultar Tienda Claro en este momento. Intenta nuevamente más tarde.",
                    "producto": [],
            ...
            # Normaliza mínimos esperados por el frontend
            if "recomendados" not in respuesta or respuesta["recomendados"] is None:
                respuesta["recomendados"] = DEFAULT_RECOMMENDATIONS
            respuesta["_meta"] = {"normalized_query": normalized_query, "category": category, "cache_hit": uso_cache}
            guardar_pregunta_en_historial(normalized_query, sia_id, respuesta, pregunta_timestamp, user_agent)
            return jsonify(respuesta)
        
        is_simple_brand_query = normalized_query in ["iphone", "samsung", "galaxy", "xiaomi", "redmi", "poco", "motorola", "moto"]
        txt = f" {normalized_query.lower()} "
        has_brand_only_block = any(f" {t} " in txt for t in BRAND_ONLY_BLOCK_TERMS)
        ...
        if respuesta is None:
            ...
            if category == "celulares":
                # Fuente de verdad: Tienda API (Cloud Run) para búsquedas de modelos / equipos.
                # (La lógica "solo marca" se resuelve antes con TOP5 en BigQuery.)
                try:
                    respuesta = call_tienda_api(normalized_query)
                except Exception as e:
                    logger.exception("Error llamando Tienda API", exc_info=e)
                ...
                # Backfill recomendados si el microservicio no lo envía
                if "recomendados" not in respuesta or respuesta["recomendados"] is None:
                    respuesta["recomendados"] = DEFAULT_RECOMMENDATIONS
            else:
...
# ✅ FALLBACK A PELÍCULAS (ÚNICO PUNTO)
        if category == "general" and respuesta:
            ...
            if is_not_found or is_clarovideo_disfruta or is_clarovideo_descripcion:
        ...
        error_msg = str(e)
        logger.error(f"[prod] Error procesando '{user_query}': {error_msg}", exc_info=True)
        return jsonify({
